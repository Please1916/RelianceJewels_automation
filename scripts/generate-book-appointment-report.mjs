// Generates a branded Book Appointment coverage PDF, spec-driven (mirrors
// generate-pdp-report.mjs).
//
// Catalog comes from the spec file:
//   - active tests : test('TC_BA_NNN | Px | name', ...)
//   - blocked tests: test.fixme('TC_BA_NNN | Px | name', ...)   -> "skip"
...
//
// Usage:
//   npx playwright test book-appointment   # writes report/results.json
//   npm run report:appointment
import { chromium } from '@playwright/test';
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import cfg from './book-appointment-report.config.mjs';
...
// Section label for a TC_BA number, by config range (first match wins).
function sectionFor(num) {
  for (const [label, [lo, hi]] of cfg.sections) if (num >= lo && num <= hi) return label;
  return cfg.defaultSection;
}

// Parse "…TC_BA_NNN… | Px | name" -> {id,num,pri,name,section}.
function parseTitle(title) {
  const idm = title.match(/TC_BA_(\d+)/);
  if (!idm) return null;
  const id = `TC_BA_${idm[1]}`;
  ...
  *{box-sizing:border-box}
  ...
  <div class="hero">
    ...
    <div class="hx">
      ...
      <div class="scope">Scope: <b>${total}</b> Book Appointment cases (P0+P1)${hasResults ? ` · executed pass rate <b>${passRate}%</b> (${tally.pass}/${tally.pass + tally.fail})` : ''}. ${escapeHtml(cfg.note || '')}</div>
    </div>
  </div>
  ...
  <footer>Reliance Jewels QA Automation · catalog source: ${escapeHtml(cfg.specPath)}${hasResults ? ` + ${escapeHtml(cfg.resultsPath)}` : ' (no run results — status shown as Automated)'} · regenerate with <code>npm run report:appointment</code></footer>
</body></html>`;
...
console.log(`Book Appointment report (${total} cases): ${summary}${hasResults ? '' : ' (no run results)'}.`);
