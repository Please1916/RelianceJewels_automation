// Generates a branded PDP coverage PDF, spec-driven.
//
// All case metadata (ID, priority, title/reason, status bucket) is parsed from
// the spec file:
//   - active tests        : test('TC_PDP_..._NNN | Px | name', ...)
//   - commented-out tests : // test('TC_PDP_..._NNN | Px | name', ...)        -> "commented"
//   - not-automatable      : // ['TC_PDP_..._NNN', 'Px', 'reason'],            -> "manual"
// Pass/skip/fail + duration for the active tests are layered in from Playwright's
// JSON results (report/results.json) when present; without it, active tests just
// show as "Automated".
//
// Usage:
//   npx playwright test "pdp-P0,P1"   # writes report/results.json (json reporter)
//   npm run report:pdp
import { chromium } from '@playwright/test';
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import cfg from './pdp-report.config.mjs';
...
const SECTION_CODES = new Set(cfg.sections.map(([c]) => c));
const ENABLED = new Set(cfg.buckets.map(([b]) => b)); // which buckets the report shows
const escapeHtml = (s) => String(s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));

// Parse "…TC_PDP_<SEC>_<NNN>… | Px | [extra |] name" -> {section,id,pri,name}.
function parseTitle(title) {
  const idm = title.match(/TC_PDP_([A-Z]+)_(\d+)/);
  if (!idm || !SECTION_CODES.has(idm[1])) return null;
  const section = idm[1];
  const id = `${section}_${idm[2]}`;
  ...
  return { section, id, pri, name };
}

const byId = new Map(); // id -> { section,id,pri,name,bucket,duration }
const put = (rec) => { if (!byId.has(rec.id)) byId.set(rec.id, rec); };

// ---- 1) Active tests + status from Playwright JSON results ----
// Only treat results.json as a real run if at least one PDP spec carries actual
// result records (a `--list` invocation writes empty `results` arrays).
let hasResults = false;
if (fs.existsSync(jsonPath)) {
  const data = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
  const pdpSpecs = [];
  const collect = (suite) => {
    for (const spec of suite.specs || []) { if (parseTitle(spec.title)) pdpSpecs.push(spec); }
    for (const c of suite.suites || []) collect(c);
  };
  (data.suites || []).forEach(collect);
  hasResults = pdpSpecs.some((s) => (s.tests?.[0]?.results?.length || 0) > 0);
  if (hasResults) {
    for (const spec of pdpSpecs) {
...
// ---- 2) Parse the spec for active titles, commented tests, manual catalog ----
const specText = fs.readFileSync(specPath, 'utf8');
for (const raw of specText.split('\n')) {
  // active test definition (only if results.json didn't already supply it).
  // Without a real run, mark as 'auto' (Automated) rather than a fake pass.
  let m = raw.match(/^\s*test(?:\.skip)?\(\s*['"`](TC_PDP_[^'"`]+)['"`]/);
  if (m) { const p = parseTitle(m[1]); if (p) put({ ...p, bucket: hasResults ? 'pass' : 'auto', duration: 0 }); continue; }

  // commented-out test definition (only if the 'commented' bucket is enabled)
  if (ENABLED.has('commented')) {
    m = raw.match(/^\s*\/\/\s*test(?:\.skip)?\(\s*['"`](TC_PDP_[^'"`]+)['"`]/);
    if (m) { const p = parseTitle(m[1]); if (p) put({ ...p, bucket: 'commented', duration: 0 }); continue; }
  }

  // not-automatable catalog entry (only if the 'manual' bucket is enabled):
  //   // ['TC_PDP_XXX_NNN', 'Px', 'reason'],
  if (ENABLED.has('manual')) {
    m = raw.match(/^\s*\/\/\s*\[\s*['"`](TC_PDP_[A-Z]+_\d+)['"`]\s*,\s*['"`](P\d)['"`]\s*,\s*['"`](.*?)['"`]\s*\]/);
    if (m) {
      const sec = m[1].match(/TC_PDP_([A-Z]+)_/)[1];
      if (SECTION_CODES.has(sec)) put({ section: sec, id: m[1].replace('TC_PDP_', ''), pri: m[2], name: m[3], bucket: 'manual', duration: 0 });
    }
  }
}

// ---- Tally ----
// Bucket set depends on whether we have a real run. Without one, the active
// tests collapse into a single neutral "Automated" bucket.
const RUN_BUCKETS = hasResults
  ? cfg.buckets
  : [['auto', '#1c958f', 'Automated'], ...cfg.buckets.filter(([b]) => b === 'commented' || b === 'manual')];
const all = [...byId.values()];
const tally = Object.fromEntries(RUN_BUCKETS.map(([b]) => [b, all.filter((r) => r.bucket === b).length]));
const total = all.length;
const active = hasResults ? (tally.pass + tally.skip + tally.fail) : tally.auto;
const passRate = hasResults && active ? Math.round((tally.pass / active) * 100) : 0;
...
// ---- Donut (SVG) ----
const donutBuckets = RUN_BUCKETS.filter(([b]) => tally[b] > 0);
const R = 60, C = 2 * Math.PI * R;
let off = 0;
const donutSegs = donutBuckets.map(([b, color]) => {
...
// Legend lists every enabled bucket (incl. zero counts); donut shows non-zero only.
...
// ---- Per-section tables ----
const BADGE_TXT = { pass: '✓ Passed', skip: '⏭ Skipped', fail: '✗ Failed', commented: '🚫 Commented', manual: '🔧 Manual', auto: 'Automated' };
const fmtMs = (ms) => (ms >= 1000 ? `${(ms / 1000).toFixed(1)}s` : ms ? `${ms}ms` : '—');
const numId = (id) => parseInt(id.split('_')[1], 10);

let sectionsHtml = '';
for (const [sec, label] of cfg.sections) {
  const rows = all.filter((r) => r.section === sec).sort((a, b) => numId(a.id) - numId(b.id));
  ...
  const stat = hasResults ? `${secPass}/${secActive.length} passed · ${rows.length} cases` : `${secActive.length} automated · ${rows.length} cases`;
  ...
  sectionsHtml += `<tr class="sec"><td colspan="5"><div class="secbar"><span>${escapeHtml(label)} <span class="seccode">TC_PDP_${sec}</span></span><span class="secstat">${stat}</span></div></td></tr>${rowsHtml}`;
}
...
const notRunning = (tally.commented || 0) + (tally.manual || 0);
const headline = hasResults
  ? `${tally.pass}/${active} automated checks pass${tally.fail ? ` · ${tally.fail} failed` : ''}${tally.skip ? ` · ${tally.skip} skipped` : ''}${notRunning ? ` · ${notRunning} tracked but not running` : ''}.`
  : `${active} cases automated${notRunning ? ` · ${notRunning} tracked but not running` : ''}.`;
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* Column widths + alignment (header and cell kept in sync) */
  .c-tc{width:78px;font-weight:700;white-space:nowrap;font-variant-numeric:tabular-nums}
  ...
  tr.sec .seccode{font-weight:500;opacity:.7;font-size:10px;margin-left:6px}
  ...
  <div class="hero">
    ...
    <div class="hx">
      ...
      <div class="scope">Scope: <b>${active}</b> automated PDP cases (P0+P1)${ENABLED.has('commented') ? ` · <b>${tally.commented || 0}</b> commented out` : ''}${ENABLED.has('manual') ? ` · <b>${tally.manual || 0}</b> manual / not-automatable` : ''}${hasResults ? ` · executed pass rate <b>${passRate}%</b> (${tally.pass}/${active})` : ''}.</div>
    </div>
  </div>
  ...
  <footer>Reliance Jewels QA Automation · catalog source: ${escapeHtml(cfg.specPath)}${hasResults ? ` + ${escapeHtml(cfg.resultsPath)}` : ' (no run results — status shown as Automated)'} · regenerate with <code>npm run report:pdp</code></footer>
</body></html>`;
...
console.log(`PDP report (${total} cases): ${summary}${hasResults ? '' : ' (no run results)'}.`);
